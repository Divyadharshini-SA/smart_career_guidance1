...
export default function Dashboard() {
  ...
  const reliabilityMsg = aptitudeTests < 5
    ? `Aptitude score based on only ${aptitudeTests} test(s). Take ${5 - aptitudeTests} more for an accurate score.`
    : technicalTests < 3
      ? `Technical score based on only ${technicalTests} test(s). Take ${3 - technicalTests} more for accuracy.`
        ...
        <GlowCard>
          ...
          {!scoreReliable && totalTests > 0 && (
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#FFD93D' }}>Score Not Yet Reliable</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>{reliabilityMsg}</div>
              </div>
            </div>
          )}
          {scoreReliable && (
